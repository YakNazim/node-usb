...
void writeURBInISOC(int fd, void *requestUniqueIDOut) {
	...
	//FIXME dangling
	struct usbdevfs_urb *myOutURB = (struct usbdevfs_urb*)malloc(structSize);
    ...
    myOutURB->iso_frame_desc[0].length = 1;
...
void reapISOC_URB(int fd) {
	...
	if (ret == -1) {
		...
		if (myURBPtr == NULL) {
			...
			if (myURBPtr->number_of_packets > 0) {
				printf("myURBPtr->iso_frame_desc[0].actual_length is %d\n", myURBPtr->iso_frame_desc[0].actual_length);
				printf("myURBPtr->iso_frame_desc[0].length is %d\n", myURBPtr->iso_frame_desc[0].length);
				printf("myURBPtr->iso_frame_desc[0].status is %d\n", myURBPtr->iso_frame_desc[0].status);
			}
...
int main(void) {
    ...
    */
	...
	for (q = 0; q < 1; q++) {
		...
		//writeURBInISOC(fd, &requestUniqueIDOut);
